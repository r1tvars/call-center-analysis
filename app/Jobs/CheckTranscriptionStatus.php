            ...
            switch ($status) {
                case 'Succeeded':
                    ...
                    if (!empty($filesResponseBody['values']) && isset($filesResponseBody['values'][0]['links']['contentUrl'])) {
                        ...
                        // Update the status and save the transcription content to the database
                        CallRecord::where('batch_id', $this->batchId)
                                  ->update(['status' => 'Succeeded', 'transcription' => $transcriptionContentJson]);
